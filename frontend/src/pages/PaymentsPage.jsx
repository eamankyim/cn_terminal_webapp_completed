  ...
  const handleSearch = (value) => {
    ...
    if (value) {
      const filtered = payments.filter(payment =>
        payment.gatewayRef?.toLowerCase().includes(value.toLowerCase()) ||
        payment.payer?.toLowerCase().includes(value.toLowerCase()) ||
        payment.invoice?.invoiceNumber?.toLowerCase().includes(value.toLowerCase())
      );
  ...
  const handleCreatePayment = async (values) => {
    try {
      if (editingPayment) {
        ...
        await paymentService.updatePayment(editingPayment.id, values);
      ...
      message.error(error.message || 'Failed to save payment');
    }
  };
  ...
  const columns = [
    {
      ...
      render: (_, record) => (
        <div>
          <div style={{ fontWeight: 'bold' }}>{record.gatewayRef || record.id}</div>
          <div style={{ fontSize: '12px', color: '#666' }}>
            Invoice: {record.invoice?.invoiceNumber || 'N/A'}
          </div>
        </div>
      ),
    },
    {
      ...
      render: (_, record) => (
        <div>
          <div style={{ fontWeight: 'bold' }}>{record.invoice?.customer?.name || 'N/A'}</div>
          <div style={{ fontSize: '12px', color: '#666' }}>
            {record.payer || 'N/A'}
          </div>
        </div>
    ...
    },
    {
      ...
      render: (_, record) => (
        <div style={{ fontWeight: 'bold', color: '#2FA2EE' }}>
          {record.currency || 'GHS'} {record.amount?.toLocaleString() || '0'}
        </div>
      ),
    },
    {
        ...
        <Row gutter={16} align="middle">
          ...
          <Col xs={24} md={4}>
            <Select
              ...
              onChange={(value) => {
                if (value) {
                  setPayments(payments.filter(p => p.status === value));
                } else {
                  loadPayments();
                }
              }}
          ...
          <Col xs={24} md={4}>
            <Select
              ...
              onChange={(value) => {
                if (value) {
                  setPayments(payments.filter(p => p.paymentMethod === value));
                } else {
                  loadPayments();
                }
              }}
